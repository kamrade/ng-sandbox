...
@Component({
  selector: 'app-query-list-child',
  templateUrl: './query-list-child.component.html',
  styleUrls: ['./query-list-child.component.scss']
})
export class QueryListChildComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.books.changes.subscribe((val) => {
  ...
  filterByGenre(genre: string) {
    this.books.filter( (elem) => {
      ...
      this.ren.addClass(bookNode.nativeElement, 'selected');
    });
  }
